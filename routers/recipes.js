...
const consumeAPIGiphy = async (jsonRecipePuppy) => {
    ...
    for await (let element of jsonRecipePuppy) {
        ...
        if (jsonGiphy.data !== undefined) {
            element.gif = jsonGiphy.data[0].images.fixed_height.url.split(
                '?'
            )[0];
        }
    }
...
router.get('/:i?', async (req, res) => {
    try {
        ...
        if (recipes.length > 3)
            throw new Error('Maximum number of ingredients exceeded');
